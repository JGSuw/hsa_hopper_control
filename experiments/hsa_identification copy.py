...
###
"""
reimagining the system identification concept
use a benchtop model or ideal model to design a trajectory

provide this trajectory as pair of interpolation functions,
r(t) and u(t) so that the motor can apply torque
Kp*(r(t)-x(t))-Kd*(x'(t))+u(t)

for some stance phase... not sure how to pick that stance phase

1) could pull it out of some ideal simulation, then just run it, and not worry about
    the size and kind of tracking error seen
2) could try iterative learning control to adjust the feed-forward term until it converges...
    if it converges

    
random thoughts;

instead of just shooting for a model of the HSA potential function, could also include
models for kinetic energy metric and rayleigh dissipation. What would this look like?

T(x,xdot) = .5 * w_i e^{(y^i-x)^2 / (2 s^2)} * xdot**2 
R(x,xdot) = .5 * w_i e^{(y^i-x)^2 / (2 s^2)} * xdot**2
V(z) = w_i e^{(y^i-x)^2 / (2 s^2)}
"""
